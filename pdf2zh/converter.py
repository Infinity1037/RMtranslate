...
# --- Updated Translator Imports ---
# Only import BaseTranslator directly and the module containing all translators
import pdf2zh.translators
from pdf2zh.translators import BaseTranslator
# --- End Updated Translator Imports ---
...
class TranslateConverter(PDFConverterEx):
    def __init__(
        ...
        # --- Dynamic Translator Instantiation ---
        translator_class = None
        # Find the class name matching the service name (case-insensitive)
        for name in pdf2zh.translators.__all__:
             if name.lower().replace("translator", "") == service_name.lower().replace("translate", "").replace(" ", ""):
                 try:
                     # Get the class object from the translators module
                     translator_class = getattr(pdf2zh.translators, name)
                     # Check if it's a subclass of BaseTranslator (and not BaseTranslator itself)
                     if issubclass(translator_class, BaseTranslator) and translator_class is not BaseTranslator:
                         break # Found the class
                     else:
                         translator_class = None # Reset if it's not a valid translator class
                 except AttributeError:
                     continue # Should not happen with __all__ but good practice
        ...
        if translator_class:
            try:
                ...
                log.info(f"Using translator: {translator_class.__name__}")
            except Exception as e:
                 log.error(f"Failed to initialize translator {translator_class.__name__}: {e}")
                 raise ValueError(f"Failed to initialize translator: {service_name}") from e
        else:
             log.error(f"Unsupported or could not find translation service class for: {service_name}")
             raise ValueError(f"Unsupported translation service: {service_name}")
        # --- End Dynamic Translator Instantiation ---
